...
int solutionSaveFrequency = 5000;
...
int trainsize = 50;//size of training set
int testsize = 50;//size of test set
int validsize = 50;
...
public:
	...
	int BackPropogationMemory(string SnapshotCount, TrainingExamples TraineeSamples, double LearningRate, Sizes Layersize, string Savefile, bool load, double *CurrentBestGradient, int *CurrentBestIndex, int CurrentIndex, double *CurrentBestError, int *CurrentBestErrorIndex, ofstream &bcout, int TotalGSTimeSpent, double *memorySnapShotsTrainRMSE);
...
*/
int NeuralNetwork::BackPropogationMemory(string SnapShotCount, TrainingExamples TraineeSamples, double LearningRate, Sizes Layersize, string Savefile, bool load, double *CurrentBestGradient, int *CurrentBestGradientIndex, int CurrentIndex, double *CurrentBestError, int *CurrentBestErrorIndex, ofstream &bcout, int TotalGSTimeSpent, double *memorySnapShotsTrainRMSE)
{
	ofstream localerrorOutput;
	localerrorOutput.open("LocalErrorOutput.txt", fstream::app);

	localerrorOutput << TotalGSTimeSpent << "\t";
	maxgenLS = (solutionSaveFrequency * (maxgen - TotalGSTimeSpent)) / maxgen;
	...
	while (Learn == true) {
		...
		cout << SumErrorSquared << " : is SumErrorSquared " << " ErrorCounter: " << minimumErrorCounter << SnapShotCount << endl;

		*memorySnapShotsTrainRMSE = SumErrorSquared;

		localerrorOutput << SumErrorSquared << "\t";

		SaveLearnedData(Layersize, Savefile);


		cout << SumErrorSquared << " " << minimumSumSquaredError << endl;
		if (SumErrorSquared < minimumSumSquaredError) {

			//Update local minium
			minimumSumSquaredError = SumErrorSquared;

			//Check if local minimum is better than global. Replace
			if (minimumSumSquaredError < *CurrentBestError)
			{
	...
	localerrorOutput << "\t EOL" << endl;
	localerrorOutput.close();
...
void GeneticAlgorithmn::InitilisePopulation(int popsize)
{
	...
	for (int i = 0; i < popsize; i++) {
		listSortedSubPops.push_back(0); //initialize with 0s
	}
...
public: //class variables
	...
	void ReplaceIndividualAtIndex(int index, vector<double>* memorySnapShot);
...
void   CoEvolution::SaveGlobalError()
{
	/*ofstream errorOutput;
	errorOutput.open("GlobalErrorOutput.txt", fstream::app);

	double test =  Species[NoSpecies - 1].Population[Species[NoSpecies - 1].BestIndex].Fitness;
...
/*
--ReplaceIndividualAtIndex--
Sort() is called before this function which ranks subpopulations by fitness in all populations
This function takes in an index to be replaced and the chrome values to replace with
e.g We want to save a solution to the 2nd rank individual. What this does is replace all 2nd rank subpopulations in all populations with the decomposed 
parameter chrome
*/
void  CoEvolution::ReplaceIndividualAtIndex(int index, vector<double>* memorySnapShot)
{
...
*/
void  CoEvolution::sort_subpops()
{

	cout << "Sorting Snapshots" << endl;
	int i, j, temp;
	double dbest;
	...
	for (int sN = 0; sN < NoSpecies; sN++)
	{
		...
		for (i = 0; i < Species[sN].listSortedSubPops.size(); i++)
			Species[sN].listSortedSubPops[i] = i;


		for (i = 0; i < (Species[sN].listSortedSubPops.size() - 1); i++)
		{
			dbest = Species[sN].Population[i].Fitness;

			for (j = i + 1; j < (Species[sN].listSortedSubPops.size()); j++)
			{
				if (Species[sN].Population[j].Fitness < dbest)
				{
					dbest = Species[sN].Population[j].Fitness;//get the best fitness
					temp = Species[sN].listSortedSubPops[j];
					Species[sN].listSortedSubPops[j] = Species[sN].listSortedSubPops[i];
					Species[sN].listSortedSubPops[i] = temp;
				}
			}
		}
	}
...
void    CombinedEvolution::ProcedureMemory(string file, bool bp, double h, ofstream &out1, ofstream &out2, ofstream &out3, double mutation, double depth, ofstream &vcout, ofstream &ecout, ofstream &bcout)
{
	...
	ofstream errorOutput;
	errorOutput.open("GlobalErrorOutput.txt");


	ofstream errorOutputFinal;
	errorOutputFinal.open("GlobalErrorOutputFinal.txt");
	...
	if (bp) {
		...
		vector<int> TotalGSTimeSpent;

		//while the maximum number of function evaluations is not reached
		while (TotalEval <= maxgen)
		{
			while ((TotalEval - previousCounter) < (solutionSaveFrequency + 1))
			{
			...
			TotalGSTimeSpent.push_back(TotalEval);
			//#######################################//
		...
		//Run LS on all saved solutions
		//LS time on each solution is different
		...
		cout << "Begin refinement of saved solutions: " << memorySnapShots.size() << " solutions in total" << endl;

		//system("pause");

		vector<double> memorySnapShotsTrainRMSE(memorySnapShots.size());
		vector<int> memorySnapShotsFitnessOrder(memorySnapShots.size());

		ofstream clear;
		clear.open("LocalErrorOutput.txt");
		clear.close();
		...
		for (int i = 0; i < memorySnapShots.size(); i++)
		{
			...
			ss << " Solution:" << (i + 1) << "/" << memorySnapShots.size();

			gene += network.BackPropogationMemory(ss.str(), Samples, 1, layersize, file, true, &currentBestGradient, &currentBestGradientIndex, i, &currentBestError, &currentBestErrorIndex, bcout, TotalGSTimeSpent[i], &memorySnapShotsTrainRMSE[i]); //use backpropogation
		...
		//--------------------------------------------------------------------------------------
		//Sort saved solutions according to fitness

		cout << "Sorting Snapshots" << endl; 
		int i, j, temp;
		double dbest;

		for (i = 0; i < memorySnapShotsTrainRMSE.size(); i++)
			memorySnapShotsFitnessOrder[i] = i;

	
			for (i = 0; i < (memorySnapShotsTrainRMSE.size() - 1); i++)
			{
				dbest = memorySnapShotsTrainRMSE[memorySnapShotsFitnessOrder[i]];

				for (j = i + 1; j < (memorySnapShotsTrainRMSE.size()); j++)
				{
					if (memorySnapShotsTrainRMSE[memorySnapShotsFitnessOrder[j]] < dbest)
					{
						dbest = memorySnapShotsTrainRMSE[memorySnapShotsFitnessOrder[j]];//get the best fitness
						temp = memorySnapShotsFitnessOrder[j];
						memorySnapShotsFitnessOrder[j] = memorySnapShotsFitnessOrder[i];
						memorySnapShotsFitnessOrder[i] = temp;
					}
				}
			}

			cout << "MemorySnapshots RMSE: " << endl;

			for (int i = 0; i < memorySnapShotsTrainRMSE.size(); i++) 
			{
				cout << "[" << i << "] " << memorySnapShotsTrainRMSE[i] << endl;

			}

			cout << "\nMemorySnapshots Order: " << endl;

			for (int i = 0; i < memorySnapShotsFitnessOrder.size(); i++)
			{
				cout << "[" << i << "] " << memorySnapShotsFitnessOrder[i] << endl;

			}

			cout << "Current SnapShots Best Index = " << currentBestErrorIndex << endl;

		//--------------------------------------------------------------------------------------

		//system("pause");

		NeuronLevel.sort_subpops();

		cout << "Replacing Top Sub-populations with refined saved solutions" << endl;

		int counter = 0; //0 = best index, 1 = 2nd best index etc.

		//for each of the memory snapshots save back into subpopulations of CCNL
		for (int y = 0; y < memorySnapShots.size(); y++) 
		{
			//ReplaceIndividualAtIndex(counter, &memorySnapShots[memorySnapShotsFitnessOrder[y]])

			//Save to network
			network.LoadSavedDataMemory(layersize, &memorySnapShots[memorySnapShotsFitnessOrder[y]]); //load from memory to network
			network.SaveLearnedData(layersize, file);//save the neural network weights for testing

			//Save to CoEvolution individual 
			NeuronLevel.Individual = network.Neurons_to_chromes(1);
			...
			//listSortedSubPops - holds the order of subpopulations
			
			//for each species
				//for each subpopulation in species
					//Replace subpopulation Population[index] where index is the current saved solution.

			//e.g
			//	subpopulations size of 10. Saved solution size of 5.
			//  1. order subpopulations
			//	2. order saved solutions
			//	3. top 5 in subpopulations will be replaced by refined individuals
			//
			//
			//
			//
			...
			{
				for (int s = 0; s < NeuronLevel.SpeciesSize[sN]; s++)
					NeuronLevel.Species[sN].Population[NeuronLevel.Species[sN].listSortedSubPops[counter]].Chrome[s] = NeuronLevel.TableSp[sN].SingleSp[s];

				//Counter = 0 meaning list[counter] basically the index in 0 is the best fitness

			/*	if (counter = 0) 
				{
					NeuronLevel.Species[sN].BestIndex = NeuronLevel.Species[sN].listSortedSubPops[counter];
				}*/
			/*	NeuronLevel.Species[sN].BestIndex = Best;*/
			}
		...
		cout << "Last Round of GS Search" << endl;

		while (TotalEval <= (maxgen * 2))
		{
			...
			for (int i = 0; i < 20; i++)
			{
				...
				errorOutputFinal << Error << "\t";
				count++;
			}


		}
		cout << "End Last Round of GS Search: Best Train Error obtained so far = " << Error << endl;
		...
		try
		{
			...
			for (double hidden = HiddenNeuronsStart; hidden <= MaxNumNeurons; hidden += HiddenIncrementBy) {//13
				...
				for (int run = 1; run <= maxrun; run++) {
					oss.str("");
					oss.clear();
					oss << "Experiment: MCNECOL-BEST Dataset " << (datasetEnumerator + 1) << "/" << (datasetNames.size()) << " - " << datasetNames[datasetEnumerator] << " Run " << run << "/" << maxrun;
